...
if ( ! function_exists( 'mai_posted_on' ) ) :
	...
	function mai_posted_on() {
		...
		// Display author name.
		$display_author = get_theme_mod( 'mai_display_author', 'no' );
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list && mai_categorized_blog() ) {
				printf( '<span class="cat-links meta-item mr-3"><i class="icon-category icon12"></i> ' . esc_html__( 'Posted in %1$s', 'mai' ) . '</span>', $categories_list );
			}
		}
		...
		if ( ! post_password_required() && ( comments_open() || $comments_number ) ) {
			...
			comments_popup_link( __( 'Leave a Comment' ), __( '1 Comment' ), esc_html__('% Comments', 'mai' ),
				'text-muted' );
			echo '</span>';
		}
 ...
 * @return bool
 */
function mai_categorized_blog() {
	$all_the_cool_cats = get_transient( 'mai_categories' );
	if ( false === $all_the_cool_cats ) {
		...
		set_transient( 'mai_categories', $all_the_cool_cats );
	}
...
/**
 * Flush out the transients used in mai_categorized_blog.
 */
function mai_category_transient_flusher() {
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
		return;
	}
	// Like, beat it. Dig?
	delete_transient( 'mai_categories' );
}

add_action( 'edit_category', 'mai_category_transient_flusher' );
add_action( 'save_post', 'mai_category_transient_flusher' );
 ...
 */
function mai_excerpt( $limit = 180 ) {
